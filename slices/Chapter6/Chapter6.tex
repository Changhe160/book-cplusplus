...
\begin{itemize}
  ...
  \item \alert{面向对象程序设计}（object-oriented programming\index{O!object-oriented programming!object}，OOP）的基础。
\end{itemize}
\end{block}

\begin{block}<2->{类的要素}
\begin{itemize}
  \item \alert{抽象}：\alert{数据（属性）}抽象和\alert{函数（操作）}抽象；
  \item \alert{封装}：将\alert{数据}和\alert{操作}结合在一起。
\end{itemize}
\end{block}
\end{frame}
...
\begin{itemize}
  ...
  \end{itemize}
  \item 需要使用\alert{作用域符}标明成员函数所属的类；
\end{itemize}
\end{block}
...
\begin{itemize}
  ...
  \item 类的使用者：隐藏类的实现细节，使使用者不能在类的外面操控数据成员，从而形成一种保护机制。
\end{itemize}
\end{block}
...
\end{lstlisting}\vspace{-1.5mm}
  ...
  \onslide<2-> 利用~\textcolor{blue}{class}~定义的类，如果没有显式指明成员的访问属性，它们的访问属性默认为~\textcolor{blue}{private}，在类的外部直接访问它们是非法的。

\end{greenblock}

\begin{redblock}<3->{\textcolor{blue}{class}~和~\textcolor{blue}{struct}}
\begin{itemize}
  \item 都可以定义一个类。
...
\begin{itemize}
  ...
  \item 类类型的成员执行自己的析构函数，内置类型的成员什么也不需要做；
...
\begin{frame}[fragile]
    {6.3.2 重载原则}
\begin{yellowblock}<1->{默认赋值运算符}
没有显式重载赋值运算符时，编译器会自动合成一个默认的赋值运算符。
\begin{itemize}
  \item 参数与返回值类型与自定义的函数一致；
  \item 使用赋值运算符将右侧对象的每个数据成员逐个赋值给左侧对象相应的数据成员；
  \item 对于数组成员，逐元素赋值。
\end{itemize}
\end{yellowblock}

\begin{redblock}<2->{提示：运算符属性继承}
\begin{itemize}
  \item 重载的运算符函数不会改变运算符的优先级、结合性以及目数等属性；
  \item 运算符的求值次序无法应用于重载的运算符。
\end{itemize}
\end{redblock}
\end{frame}
...
\begin{greenblock}<3->{问题：为什么要返回~ostream~类对象的引用？}
\begin{itemize}
  \item <4->[] 因为可以实现连续输出：
\begin{lstlisting}
Fraction a, b;
...
\begin{greenblock}<1->{下面程序是否有问题？应如何修改？}
\begin{lstlisting}
class Fraction {
    int m_numerator = 0;
    int m_denominator = 1;
...
\begin{frame}[fragile]
    {6.3.4 递增和递减运算符}
\begin{greenblock}{问题：为什么后置版本采用一个额外的~int~形参？}
\begin{itemize}
  \item<2->[]
  区别前置和后置版本。
\end{itemize}
...
Fraction f = 2;
\end{lstlisting}\vspace{-1.5mm}
  \item<2->[]正确，调用默认构造函数，将  \textcolor{blue}{int} 类型隐式转换成 Fraction 类型。
  \item<3->[]
  \begin{lstlisting}
double x = 1.5 + f;
\end{lstlisting}\vspace{-1.5mm}
