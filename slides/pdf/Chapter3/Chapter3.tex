...
\begin{frame}[fragile]
    ...
    /*...*/
                \end{lstlisting}
		A.死循环~~B.执行3次~~C.执行一次~~D.有语法错误\\
		\ttfamily 2.下面程序段的输出结果是？\\
		\begin{lstlisting}[basicstyle=\small\ttfamily]
    int y = 10;
    do{
        y--;
...
\begin{frame}[fragile]
	...
	\begin{columns}
		\begin{column}{0.4\textwidth}
			\begin{figure}[ht]
				\centering
				\includegraphics[scale=0.28]{example3_8.png}
		...
		\begin{column}{0.4\textwidth}
			\begin{figure}[ht]
				\centering
				\includegraphics[scale=0.18]{Fig3-3.png}
