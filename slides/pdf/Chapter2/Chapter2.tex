...
\column{0.9\textwidth}
\begin{block}{C++字符集}
 包括大小写字母、阿拉伯数字以及符号：\verb;+;~~~~\verb;–;~~~~\verb;*;~~~~\verb;/;~~~~\verb;=;~~~~\verb;,;~~~~\verb;.;~~~~\verb;_;~~~~ \verb;:;~~~~ \verb;;;~~~~ \verb;?;~~~~ \verb;";~~~~ \verb;';~~~~ \verb;~; ~~~~\verb;|;~~~~ \verb;!;~~~~ \verb;#; ~~~~\verb;%;~~~~ \verb;&;~~~~ \verb;();~~~~ \verb;[];~~~~ \verb;{};~~~~ \verb;^; ~~~~\verb;<; ~~~~\verb;>;和空格等
\end{block}
~\\
\begin{block}<2->{标识符}
	\begin{itemize}
        \item 是由用户定义的“单词”，用来给变量、常量、数据类型、函数等命名
		\item 合法的标识符由\alert{字母}、\alert{数字}和\alert{下划线}组成，且必须以\alert{字母}或\alert{下划线}开头
		\item 严格区分大小写，如:name、Name和NAME是三个不同的标识符
	\end{itemize}
\end{block}
\end{columns}
\end{frame}
...
\begin{block}{左值和右值}
	\begin{itemize}
        ...
        \item 左值对象由程序员创建并命名，具有持久性。右值对象中除了字面值常量以外，都是临时对象
...
\begin{figure}[!h]
\centering
\includegraphics[width=\textwidth]{biaodashi.png}
\end{figure}
\end{yellowblock}
\end{columns}
\end{frame}
...
2.答案一：\texttt{a==0\texttt{\&\&}b\texttt{!}=0\texttt{||}a\texttt{!}=0\texttt{\&\&}==0}\\
...
%---------------------------------------------------------------------------------------------
\begin{frame}[fragile]{2.5 表达式\normalsize{~---~逗号运算符}}
逗号表达式：由\alert{逗号运算符}连接起来的表达式。从左向右依次计算每个运算对象，结果为最右边的运算对象
\begin{columns}[t]
...
\begin{itemize}
  ...
  \item 左侧代码中，\texttt{i}的值为11，\texttt{j}的值为9
\end{itemize}
\end{yellowblock}
\end{columns}
\end{frame}
...
\begin{frame}[fragile]{2.5 表达式\normalsize{~---~位运算符}}
\begin{itemize}
  \item \alert{位运算符}：~\verb;~;~、\verb;<<;~、\verb;>>;~、\texttt{\&}~、\texttt{|} ~、~\verb;^;（\alert{优先级由高到低}）
  \item 运算的对象是整型对象，处理二进制数
\end{itemize}
\pause
\begin{columns}[t]
...
&&11111111~11111010&&&00000000~00001010&&&00000000~00000010 \vspace{0.3cm}\\
...
\alert{显示转换方式}：\\
\begin{itemize}
  \item \texttt{static\_cast}、\texttt{dynamic\_cast}、\texttt{const\_cast}、\texttt{reinterpret\_cast}
\end{itemize}
\pause
