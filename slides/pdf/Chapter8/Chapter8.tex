...
\uncover<3->{
释放一块非\texttt{new}分配的内存、同一内存多次释放或者使用一个已经释放的内存，其行为都是未定义的：}
...
\end{yellowblock}
\begin{redblock}<4->{注意}
内存泄漏难捉摸、难检测，导致性能逐渐降低，耗尽内存，甚至使另一个程序运行失败
\end{redblock}
...
\uncover<3->{智能指针的使用和普通指针类似，解引用时返回其指向的对象：}
...
\column{0.3\textwidth}
\begin{yellowblock}<2->{说明}
\texttt{release}函数将\texttt{p1}置为\texttt{nullptr}并返回\texttt{p1}原来的指针
\end{yellowblock}
\begin{yellowblock}<3->{说明}
\texttt{reset}函数释放\texttt{p3}原来的动态内存，并指向\texttt{p2}释放出来的内存
\end{yellowblock}
\end{columns}
...
$\bullet$~\texttt{s1}、\texttt{s2}和\texttt{s3}的\texttt{m\_buff}指向同一个内存地址，当它们消亡时，同一个动态内存将会被释放多次
\end{yellowblock}
...
$\bullet$~若缺少判断，当被复制对象为自身时，\texttt{delete}语句会在复制数据之前将对象销毁，引发错误\\
$\bullet$~此外也避免了对自己的复制
\end{greenblock}
...
移动构造中若没有将临时对象\texttt{rhs}的指针成员置为空指针，会有什么后果？
\end{greenblock}
\vspace{-2mm}
\begin{greenblock}<4->{答案}
\texttt{rhs}析构时会释放动态内存，导致新创建对象的\texttt{m\_buff}成为野指针
\end{greenblock}
...
$\bullet$~如果\texttt{pos}为尾结点，需要修改尾指针使其指向新结点
\end{yellowblock}
\vspace{-2mm}
\begin{redblock}<3->{注意}
...
\end{yellowblock}
\begin{yellowblock}<2->{说明}
从根结点开始，如果待插入结点的数据值小于根结点的数据值，则在其左子树上查找要插入的位置；否则在其右子树中查找
\end{yellowblock}
...
\vspace{-2mm}
\begin{greenblock}<2->{问题}
第一个形参\alert{必须}为\texttt{Node}类型指针的\alert{引用}，而不是\texttt{Node}类型的指针。为什么？
\end{greenblock}
\vspace{-2mm}
\begin{greenblock}<3->{答案}
如果不是引用，创建新结点时，新的动态内存虽然被分配了，但只有局部对象\texttt{p}被改指向这个新地址，真正的实参的值还是\texttt{nullptr}
\end{greenblock}
...
\begin{block}<23->{后序遍历}
左子树-->右子树-->\alert{根结点}\\
\texttt{\uncover<24->{1} \uncover<25->{3} \uncover<26->{4} \uncover<27->{6} \uncover<28->{7} \uncover<29->{8} \uncover<30->{10} \uncover<31->{13} \uncover<32->{14}}
\end{block}
...
\begin{yellowblock}{说明}
\texttt{inOrder}函数的第二个参数，为用户自定义的访问处理函数，类型为一个指向返回值为空、包含一个\texttt{Node}类型的引用形参的函数指针
\end{yellowblock}
...
\end{yellowblock}
\begin{redblock}<2->{注意}
若有\alert{其他成员函数}里执行此函数，必须把给定结点的父结点（如果有的话）中对应的指针域\alert{置空}，否则其父结点相应的指针为空悬指针
\end{redblock}
