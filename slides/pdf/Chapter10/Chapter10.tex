...
cout<<a<<endl;
	...
	\begin{frame}[fragile]{10.1.1~IO~类对象}
		...
		\begin{block}{~IO~类关系}
			\begin{itemize}
				\item 类名中的~i~和~o~分别代表~input~和~output~。
			\end{itemize}
		\vspace{-4ex}
			\begin{center}
...
cout<<"x:"<<setw(10)<<x<<endl;
	...
	\begin{blueblock}{输出结果：}
		...
		$x:\sqcup$$\sqcup$$\sqcup$$\sqcup\, 1$\hspace{0.5ex}$.$\hspace{0.5ex}$2$\hspace{0.1ex} $1$$52$
	\end{blueblock}
...
\subsection{文件模式}
	...
	\begin{frame}[fragile]{10.3.2~文件模式}
		...
		\begin{blueblock}{代码清单 10.1，例 10.1：}
...
int main() {
	...
	\end{frame}
	\begin{frame}[fragile]{10.3.2~文件模式}
	\begin{blueblock}{代码清单 10.1，例 10.1 百鸡问题代码\alert{续}：}
	\begin{lstlisting}[moreemph={Array,T,Less,F}]
