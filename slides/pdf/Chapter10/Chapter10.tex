...
%		\end{columns}
	...
	\begin{frame}[fragile]{10.1~基本知识}
		...
		\begin{block}{C++的IO操作}
			\vspace{2ex}
			\begin{itemize}
				\item C++~语言不能直接处理~IO~操作，依靠不同的~IO~类来实现从设备中读取数据和向设备写入数据。\\
				\vspace{4ex}
				\item 
	...
	\begin{frame}[fragile]{10.1.1~IO~类对象}
		...
		\begin{block}{常用的~IO~类}
			\begin{itemize}
				\item 		常用的~IO~类有~itream~和~ostream~，包括文件流类型和~string~流类型。\\\vspace{2ex}
				\item  
				ios~是抽象基类，由它派生出~istream~类和~ostream~类。\\\vspace{2ex}
				\item 
				ifstream~和~istringstream~继承~istream；~ofstream~和~ostringstream~继承~ostream。
			\end{itemize}
		\end{block}
...
cin>>x;
		...
		\end{blueblock}
		\begin{greenblock}<2->{}
		cin~会进入错误状态，它就变成无效的，无法再执行后续的输入。因此，在使用~cin~时，要确保它的状态是有效的。
		\end{greenblock}
		\begin{blueblock}{有效性判断：}<3->
		\begin{lstlisting}[moreemph={Array,T,Less,F}]
...
cout<<"ends"<<ends;//输出ends和一个空字符，然后刷新缓冲区
	...
	\begin{frame}[fragile]{10.2.1~字符数据的输入}
		\begin{blueblock}{使用cin~>>~操作：}
			数据的输入一般以空白字符结束（\alert{包括空格符、制表符和回车符}等），而这些空白字符会被系统过滤掉。
		\end{blueblock}
		\vspace{4ex}
		\begin{blueblock}{使用cin.get()函数：}
			cin.get()~函数从输入流中获取一个字符，并将其返回。
		\end{blueblock}
...
precision:3,x=1.22
precision:4,x=1.215
scientific:scientific: 2.718282e+01
fixed decimal: 27.182818
...
int main() {
	...
	\end{frame}
	\begin{frame}[fragile]{10.3.2~文件模式}
	\begin{blueblock}{百鸡问题代码\alert{续}：}
	\begin{lstlisting}[moreemph={Array,T,Less,F}]
...
\subsection{~string~流}
	\begin{frame}[fragile]{~string~流}
		string~流可以向~string~类对象读写数据。string~流定义在~sstream~头文件中，它包含三个类型：istringstream、~ostringstream~和~stringstream。
		\vspace{4ex}
