...
$\bullet$ 如果列表含有多个模板参数，则参数之间用\alert{逗号}分开
\end{yellowblock}
\begin{redblock}<3->{注意}
模板的声明和定义应放在同一个头文件里
\end{redblock}
...
\vspace{-2mm}
\begin{yellowblock}{说明}
模板实参\texttt{T}的类型为\texttt{const char*}，函数形参\texttt{a}和\texttt{b}分别为一个指向\texttt{const char}对象的\texttt{const}指针的引用，函数体里面的操作是两个字符串值的比较
\end{yellowblock}
\vspace{-2mm}
\begin{redblock}<2->{注意}
...
\end{yellowblock}
\begin{yellowblock}<2->{说明}
$\bullet$ 第一条\texttt{reset}函数调用将模板成员函数的模板参数\texttt{T}与\texttt{int}类型绑定，\texttt{m\_p}存放整型对象\texttt{i}的地址\\
$\bullet$ 第二条\texttt{reset}函数调用，编译器推断出模板成员函数的模板参数\texttt{T}为\texttt{double}类型，并重置\texttt{m\_p}的值为\texttt{double}类型对象\texttt{d}的地址
\end{yellowblock}
...
\column{0.3\textwidth}
\begin{yellowblock}{说明}
$\bullet$ 如果\texttt{value}小于中点位置（\texttt{middle}）元素，则将\texttt{right}指针设置为\texttt{middle-1}；\\
$\bullet$ 如果\texttt{value}大于中点位置元素，则将\texttt{left}指针设置为\texttt{middle+1}；\\
...
\vspace{-2mm}
\begin{greenblock}<3->{答案}
查找失败，结束于元素\texttt{1}，\texttt{left}值为\texttt{2}，\texttt{right}值为\texttt{1}
\end{greenblock}
