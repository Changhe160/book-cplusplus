...
\end{block}
    ...
    \begin{blueblock}<2->{\texttt{maxnum}函数:返回两个整数中较大的数}
\vspace{-1.5mm}\begin{lstlisting}
int maxnum(int a, int b) { //a和b为两个int类型(*@\alert{形参}@*)
     int c;                //用来保存结果
     c=a > b ? a : b;
     return c              //返回结果
}
...
\column{0.5\textwidth}
    ...
    \begin{blueblock}{通过函数名\texttt{maxnum} 作用于两个实参$x$和$y$来调用\texttt{maxnum}函数。}
\vspace{-1.5mm}\begin{lstlisting}
int main() {
    int x, y, z;
    cin >> x >> y;
...
函数调用过程（如下）：
            ...
            \begin{figure}[ht]
                \centering
                \includegraphics[scale=0.7]{FIG/Fig5-1}
...
\begin{columns}
\column{0.3\textwidth}
    \begin{blueblock}{\texttt{maxnum}函数}
\vspace{-1.5mm}\begin{lstlisting}
...
\begin{greenblock}{是否正确调用\texttt{maxnum}函数。}
...
\begin{itemize}
  \item<3->[]若主调函数和被调函数间无信息传递，如何设置函数的形参和返回值？
  \item<4->[]使用无参列表和 void 返回类型。
\end{itemize}
\end{greenblock}

\begin{blueblock}<5->{无参列表和~void~返回类型例子}
\vspace{-1.5mm}\begin{lstlisting}
...
\begin{block}<2->{函数声明的要素}
函数的\alert{返回类型}、\alert{名字}和\alert{形参类型}。
\end{block}
...
\begin{frame}[fragile]
    {5.2 认识函数\small{~---~局部对象}}
\begin{block}{局部对象}
在语句块内部定义的对象称为\alert{局部对象}（local object\index{L!local object}），包括函数的形参。
\end{block}
\begin{redblock}{局部对象}
局部对象仅在相应的语句块内部可见，而且还会屏蔽外层作用域中的同名对象。
\end{redblock}
\end{frame}
...
\begin{block}{链接性（linkage\index{L!linkage}）：如何共享}
\begin{itemize}
  \item 局部对象没有链接性，不能共享；
...
\begin{columns}
\column{0.45\textwidth}
\begin{blueblock}<1->{示例1}\vspace{-3.5mm}\begin{lstlisting}
void Swap(int *x, int *y) {
    int z(*x);
    *x = *y;
    *y = z;
...
\begin{figure}
  \centering
  \includegraphics[width=0.6\textwidth]{FIG/exp5-3-1-2-a}
\end{figure}
...
\begin{columns}
\column{0.45\textwidth}
\begin{blueblock}<1->{示例2}\vspace{-3.5mm}\begin{lstlisting}
void Swap(int *x, int *y) {
    int *z(x);
    x = y;
    y = z;
...
\begin{figure}
  \centering
  \includegraphics[width=0.6\textwidth]{FIG/exp5-3-1-2-b}
\end{figure}
...
\end{blueblock}
\column{0.45\textwidth}
\begin{blueblock}<3->{利用指针标明访问的数组元素的范围}\vspace{-1.5mm}\begin{lstlisting}
...
\vspace{-2.5mm}\begin{lstlisting}
turnoff(); //省略实参，形参使用默认值
turnoff(22); //包含实参，形参接受实参值
\end{lstlisting}\vspace{-1.5mm}
\end{blueblock}
\end{columns}
...
\end{itemize}
C++~提供了一种即保证函数化的形式又兼顾程序的执行效率的方法，即内联函数。
\end{block}
\begin{blueblock}<2->{示例}
\vspace{-2.5mm}\begin{lstlisting}
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{FIG/Fig7-2}
\end{figure}
...
\subsection{constexpr~函数}\label{chap5-5-4}
\begin{frame}[fragile]
...
\end{blueblock}
\begin{redblock}<3->{注意}
constexpr~函数会隐式转化为内联函数，应该放头文件中。
\end{redblock}
\end{frame}
...
\begin{itemize}
  \item[]<3->
            \begin{figure}[ht]
                \centering
                \includegraphics[width=0.4\textwidth]{FIG/Fig5-2}
                \caption{梯形法求积分示例图}
                % \label{fig:label}
            \end{figure}
\end{itemize}
...
\begin{blueblock}<2->{省略~return type~说明}
\vspace{-2.5mm}\begin{lstlisting}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.5\textwidth]{FIG/Fig5-3}
...
\begin{itemize}
  ...
  \begin{figure}[h]
\centering
 \includegraphics[width=0.5\textwidth]{FIG/Fig5-4}
\end{figure}
\end{itemize}
...
\begin{itemize}
  \item<2->[]
  	\begin{figure}
		\centering
		\animategraphics[scale=0.2,autoplay,loop]{1}{FIG/exp5-7/exp5-7-1-}{1}{8}
        \caption{移动过程}
	\end{figure}
\end{itemize}
\end{columns}
...
\begin{itemize}
 ...
 % \item 头文件保护：与~\#endif~之间、\#pragma once~；
  \item 头文件保护：~\#ifndef、\#define 、~\#endif~预编译指令或者\#pragma once宏；
  \item 头文件包含：函数的声明、~const~对象、全局对象的声明、内联函数、~constexpr~函数、类和模版的定义。
\end{itemize}
\end{block}
